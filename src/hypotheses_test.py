...
class Hypothesis:
    ...
    def collect_mutations_per_gene(self, ancestor, intermediate, intermediate_nucl, leaf, leaf_nucl, edited_leaves = None, non_edited_leaves=None,
        ...
        for i in set(list(self.nucl_mat['super_orthologs_id'].values)):
            ...
            syn_mat = orthologs_mat[orthologs_mat[intermediate+'_'+intermediate_nucl+leaf_nucl+'_recoding']==0]
            nonsyn_mat = orthologs_mat[orthologs_mat[intermediate + '_' + intermediate_nucl + leaf_nucl + '_recoding']==1]
            syn_editing_sites = editing_sites_mat[editing_sites_mat[intermediate+'_'+intermediate_nucl+leaf_nucl+'_recoding']==0]
        ...
        if branch in self.mutations_per_gene.keys():
            self.mutations_per_gene[branch].update({mm:pd.DataFrame(data=data, columns=columns)})
        else:
            self.mutations_per_gene.update({branch:{mm:pd.DataFrame(data=data, columns=columns)}})
    ...
    def write_data(self, path, file_name='hypotheses_analysis', data_to_write = ['strict','liberal','no_depletion'], file_type='xlsx', sub_name='sheet1'):
        ...
        for data in data_to_write:
            ...
            elif data in ['mutations_per_gene']:
                if data=='mutations_per_gene':
                    for p,mm_dict in self.mutations_per_gene.items():
                        for mm, df in mm_dict.items():
                            name=file_name+'_'+p+'_'+mm
                            write(file_type,df,path,name)
