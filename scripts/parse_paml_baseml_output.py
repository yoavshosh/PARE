...
#    tree_file = 'C:/Users/shosh/OneDrive/Desktop/parent_test_dir/super_ortholog_proteins_fasta_files_all8/codons_msa_results/full_tree.txt'
    ...
    for d in dirs:
        ...
        #finding all sequences includin ancestral reconstructed sequences and storing in a list
        for i,l in enumerate(content):
        ...
        ance_nucl_prob_regex_str = '(?<=[A-Z][(])(.*?)(?=[)])'
        ance_nucl_prob_regex = re.compile(ance_nucl_prob_regex_str)
        data = []
        sorted_original_ance = [x for _,x in sorted(zip([int(k) for k in paml_ance_in_tree_srts],original_ance_in_tree_strs))]        
        columns = ['position_base_0','frequency','leafs_data']+[a+'_prob' for a in sorted_original_ance]
        for i,l in enumerate(content):
        ...
        for j,l in enumerate(content[i+4:]):
            ...
            while '  ' in line:
                line = line.replace('  ',' ')
            line = line.split(' ')
            position_data+=(int(line[0])-1,int(line[1]),line[2].replace(':',''),)
            for nucl_prob in line[3:]:
                position_data+=(float(ance_nucl_prob_regex.search(nucl_prob).group()),)
    ...
    if len(set(branches))>1:
        print('WARNING!! Brances are not in identical order for all mlb files!')
    branches=branches[0]
    while '  ' in branches:
    ...
    translated_branches=[]
    all_nods = original_animals_in_tree_strs+sorted_original_ance
