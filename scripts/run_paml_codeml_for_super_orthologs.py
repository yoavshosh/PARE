...
for d in $(ls | grep -v txt); do cd $d; ctl=$(ls | grep ctl); echo "nohup ~/PAML4/paml4.8/bin/codeml "$ctl" > nohup_codeml_log" | sh; cd ..; done &
...
if __name__ == '__main__':
    ...
    run_parser.add_argument('-codeml_processes', dest='codeml_proc', action='store', default = '50', help='number of processes to run in parallel')
    run_parser.add_argument('-msa_fmt', dest='msa_fmt', action='store', default='clustal', help='msa format')
    run_parser.add_argument('-codeml_model', dest='codeml_model', action='store', default='0', help='codeml_model')
    
    arguments = parser.parse_args()
    
    msas_dirs_path = arguments.msas_dirs_path
    tree_file = arguments.tree_file
    codeml_proc = int(arguments.codeml_proc)
    msa_fmt = arguments.msa_fmt
    codeml_model = arguments.codeml_model
    ...
    while pennding_processes:
        if sum([proc.poll() is None for proc in paml4_processes]) < codeml_proc:
            d=dirs[current_proc]
            dn = d.split('/')[-2]
            ...
            ctl.write('runmode = 0'+'\n')
            ctl.write('seqtype = 1'+'\n')
            ctl.write('CodonFreq = 2'+'\n')
            ctl.write('*ndata = 10'+'\n')
            ctl.write('clock = 0'+'\n')
            ctl.write('aaDist = 0'+'\n')
            ctl.write('aaRatefile = dat/jones.dat'+'\n')
            ctl.write('model = '+codeml_model+'\n')
            ctl.write('NSsites = 0'+'\n')
            ctl.write('icode = 0'+'\n')
            ctl.write('Mgene = 0'+'\n')
            ctl.write('fix_kappa = 0'+'\n')
            ctl.write('kappa = 2'+'\n')
            ctl.write('fix_omega = 0'+'\n')
            ctl.write('omega = .4'+'\n')
            ctl.write('fix_alpha = 1'+'\n')
            ctl.write('alpha = 0'+'\n')
            ctl.write('Malpha = 0'+'\n')
            ctl.write('ncatG = 8'+'\n')
            ctl.write('getSE = 0'+'\n')
            ctl.write('RateAncestor = 1'+'\n')
            ctl.write('Small_Diff = .5e-6'+'\n')
            ctl.write('cleandata = 0'+'\n')
            ctl.write('*fix_blength = -1'+'\n')
            ctl.write('method = 0'+'\n')
            ctl.close()
            try:
                paml_cmd = 'nohup ~/PAML4/paml4.8/bin/codeml ./' + ctl_file + ' > ' + d+'nohup_codeml_log'
        ...
        if len(dirs)==current_proc:
            pennding_processes = False
                
    #wait until all processes are finished      
    while sum([proc.poll() is None for proc in paml4_processes]):
        time.sleep(1)
    print('Finished')
