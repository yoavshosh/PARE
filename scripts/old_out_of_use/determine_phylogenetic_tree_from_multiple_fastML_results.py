...
def run_perliminary_fastML_analysis(msa_path, fastML_proc, seq_type):
    ...
    processes = []
    files_list = glob.glob(os.path.join(msa_path,'msa_results_for_super_orthologs*.fasta'))
...
if __name__ == '__main__':
    ...
    run_parser.add_argument('-tree_subpath_str', dest='tree_subpath_str', action='store', default = 'msa_results_for_super_orthologs_*.fasta.raxml.bestTree', help='sub path of trees within msa path')
    ...
    if run_fastML:
        run_perliminary_fastML_analysis(msa_path, fastML_proc, seq_type)
