...
"""
some tutorials for orthomcl 
"""



def compare_columns(consol, columns_sufix = ['_x','_y'], animals = ['sep','squ']):
    
    exist_in_both_not_equal = []
    exits_in_both_equal = []
    T2_not_exist_in_1 = []
    O1_not_exist_in_2 = []
    
    columns = [animals[0]+'_seq_id',
               animals[0]+'_protein',
               animals[0]+'_position',
               animals[0]+'_nuc_mm',
               animals[0]+'_aa_mm',
               animals[0]+'_editing_level',
               animals[0]+'_rna_coverage',
               animals[1]+'_seq_id',
               animals[1]+'_protein',
               animals[1]+'_position',
               animals[1]+'_nuc_mm',
               animals[1]+'_aa_mm',
               animals[1]+'_editing_level',
               animals[1]+'_rna_coverage',
               animals[0]+'_key',
               animals[1]+'_key']
    
    for index, row in consol.iterrows():
        
        if all([True if (row[col+columns_sufix[0]] != row[col+columns_sufix[1]] and row.notnull()[col+columns_sufix[0]] and row.notnull()[col+columns_sufix[1]]) else False for col in columns]):
                exist_in_both_not_equal.append(row['key_for_cons'])
        elif all([True if (row.notnull()[col+columns_sufix[0]] and row.isnull()[col+columns_sufix[1]]) else False for col in columns]):
            T2_not_exist_in_1.append(row['key_for_cons'])
        elif all([True if (row.isnull()[col+columns_sufix[0]] and row.notnull()[col+columns_sufix[1]]) else False for col in columns]):
            O1_not_exist_in_2.append(row['key_for_cons'])
        elif all([True if row[col+columns_sufix[0]] == row[col+columns_sufix[1]] else False for col in columns]):
            exits_in_both_equal.append(row['key_for_cons'])
                
        
    return exits_in_both_equal, exist_in_both_not_equal, T2_not_exist_in_1, O1_not_exist_in_2
                


def consolidate_mm_files(file1, file2):
    
    df1, animal_11, animal_12 = get_orthologs_data(file1)
    df1[animal_11+'_'+animal_12+'_key_for_cons'] = df1.apply(lambda x: x[animal_11+'_key'] + '_' + x[animal_12+'_key'], axis = 1)
    df2, animal_21, animal_22 = get_orthologs_data(file2)
    df2[animal_22+'_'+animal_21+'_key_for_cons'] = df2.apply(lambda x: x[animal_11+'_key'] + '_' + x[animal_12+'_key'], axis = 1)
    
#    consol = df1.merge(df2,left_on = 'key_for_cons', right_on = 'key_for_cons', how = 'outer')
    consol = pd.concat([df1,df2],sort=False)
    consol.drop([animal_21+'_'+animal_22+'_recoding', animal_21+'_'+animal_22+'_alignment_identity'], axis = 1, inplace = True)
    consol.drop_duplicates(animal_11+'_'+animal_12+'_key_for_cons', inplace = True)
    
#    list(df1.columns)
    
    return consol, df1, df2
...
def get_orthologs_data(match_file):
...
def plot_conservations_sites_all_pairs(output_folder,all_dfs_dict, recoding = False, pairs_to_plot = []):
    ...
    colo = ['y','r','b','g','m','tan','silver','yellow','crimson','teal','k','orange','brown','limegreen']
...
#            print('pair ' + animals[0] + animals[1])
            ...
            axes_dict[item[0]] = ax.bar(ind + width*locations[i], consereved_editing_types, width, color=colo[i])
...
#    input_path = 'E:/RNA_editing_Large_files/orthomcl/compliantFasta_noa_with_lin/from_noas_folder/results/'
    input_path = 'E:/RNA_editing_Large_files/orthomcl/compliantFasta_noa_with_lin/results/'    
    
#    animals = ['bim','squ','oct','sep'] #animals strings as apear in file names (suppose to be identical to animals string within the file but not necessarily)
#    animals = ['oct','lin','sep','squ','oct']
    animals = ['bim','squ','sep','oct']
        ...
        for j in range(len(animals_deq)):
            ...
            try:
                ...
                if len(df_names_list):
                    ...
                    elif animal_1_name + '_key' in df_all_animals.columns and animal_2_name + '_key' not in df_all_animals.columns:
                    ...
                    elif animal_2_name + '_key' in df_all_animals.columns and animal_1_name + '_key' not in df_all_animals.columns:
...
#        print(len(df_all_animals_recoding))
    ...
    df_all_animals.to_excel(excel_writer, sheet_name='conserved_across_all',startrow=0 , startcol=0, index=False)
    excel_writer.save()

    
#sns.regplot(squ,sep, color ='red', ci = 99)